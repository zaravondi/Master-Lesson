...
class SpaceInvader(object):
    ...
    def train(self, num_frames):
        ...
        while observation_num < num_frames:
            ...
            predict_movement, predict_q_value = self.deep_q.predict_movement_Boltzmann(curr_state, epsilon)
            ...
            if observation_num % 10 == 0:
                print("Predicted q value = ", predict_q_value)
            ...
            self.summary_writer.add_scalar('Predicted Q Value', predict_q_value, global_step=observation_num)
            self.summary_writer.add_scalar('total_reward', total_reward, global_step=observation_num)
    ...
    def simulate(self, path = "", save = False):
        ...
        while not done:
            ...
            predict_movement = self.deep_q.predict_movement_Boltzmann(state, 0)[0]
            self.env.render()
    ...
    def calculate_mean(self, num_samples = 100):
        ...
        for i in range(num_samples):
            ...
            while not done:
                state = self.convert_process_buffer()
                predict_movement = self.deep_q.predict_movement_Boltzmann(state, 0.0)[0]
                observation, reward, done, _ = self.env.step(predict_movement)
                tot_award += reward
                self.process_buffer.append(observation)
                self.process_buffer = self.process_buffer[1:]
